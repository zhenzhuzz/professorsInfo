## [UCLA](https://www.ucla.edu/) 
[ME: Dec, 1](https://grad.ucla.edu/programs/school-of-engineering-and-applied-science/mechanical-aerospace-engineering-department/mechanical-engineering/);
[Aero: Dec, 1](https://grad.ucla.edu/programs/school-of-engineering-and-applied-science/mechanical-aerospace-engineering-department/aerospace-engineering/); 
[ECE: Dec, 15](https://grad.ucla.edu/programs/school-of-engineering-and-applied-science/electrical-computer-engineering/);
[Admission Catolog](https://grad.ucla.edu/admissions/calendar/#:~:text=Most%20graduate%20admissions%20applications%20are,some%20have%20multiple%20admissions%20rounds.);
[$155](https://grad.ucla.edu/admissions/faqs/);
[Only one program](https://grad.ucla.edu/admissions/faqs/);
[Apply Now](https://grad.ucla.edu/admissions/admission-application-for-graduate-admission/)


[Colleges and Schools](https://www.ucla.edu/academics/college-and-schools)  
[All Departments](https://grad.ucla.edu/schools/)  
[School of Engineering](https://samueli.ucla.edu/)  
[Departments in UCLA Engineering](https://samueli.ucla.edu/research-and-academics/)  

### [Mechanical & Aerospace Engineering](https://www.mae.ucla.edu/)

- **Field**: [Design, Robotics, and Manufacturing](https://www.mae.ucla.edu/faculty-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/greg-carman/research-overview.md">
                <img src="research-quickview/ucla/profiles/greg-carman-profile.jpg" width="150" style="border-radius: 8px;" alt="Greg Carman"><br>
                <b>Greg Carman</b>
            </a>
            <p>
                纳米级多铁材料, 压电材料, 磁致伸缩材料, 薄膜形状记忆合金, 光纤传感器, 智能材料与结构
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/tyler-clites/research-overview.md">
                <img src="research-quickview/ucla/profiles/tyler-clites-profile.jpg" width="150" style="border-radius: 8px;" alt="Tyler Clites"><br>
                <b>Tyler Clites</b>
            </a>
            <p>
                仿生系统, 神经接口与控制, 骨科与整形外科手术技术, 人体功能增强与康复, 软体机器人
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/anushri-dixit/research-overview.md">
                <img src="research-quickview/ucla/profiles/anushri-dixit-profile.jpg" width="150" style="border-radius: 8px;" alt="Anushri Dixit"><br>
                <b>Anushri Dixit</b>
            </a>
            <p>
                自主性, 机器人学, 安全关键决策, 随机控制, 模型预测控制, 风险感知规划
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/elisa-franco/research-overview.md">
                <img src="research-quickview/ucla/profiles/elisa-franco-profile.jpg" width="150" style="border-radius: 8px;" alt="Elisa Franco"><br>
                <b>Elisa Franco</b>
            </a>
            <p>
                结构生物学, 动态与控制, 生物分子框架, 合成生物学, 智能材料
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/rajit-gadh/research-overview.md">
                <img src="research-quickview/ucla/profiles/rajit-gadh-profile.jpg" width="150" style="border-radius: 8px;" alt="Rajit Gadh"><br>
                <b>Rajit Gadh</b>
            </a>
            <p>
                智能电网, 电动汽车与电网集成, 微电网, 分布式能源资源, 可再生能源, 自动化设计, 物联网
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/dennis-hong/research-overview.md">
                <img src="research-quickview/ucla/profiles/dennis-hong-profile.jpg" width="150" style="border-radius: 8px;" alt="Dennis Hong"><br>
                <b>Dennis Hong</b>
            </a>
            <p>
                仿生机器人, 人形机器人, 机器人操控, 软驱动器, 机器人平台, 机械设计与运动学
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/jonathan-hopkins/research-overview.md">
                <img src="research-quickview/ucla/profiles/jonathan-hopkins-profile.jpg" width="150" style="border-radius: 8px;" alt="Jonathan B. Hopkins"><br>
                <b>Jonathan B. Hopkins</b>
            </a>
            <p>
                柔性结构设计, 精密制造, 微结构分析, 智能材料, 高效折叠结构
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/m-khalid-jawed/research-overview.md">
                <img src="research-quickview/ucla/profiles/m-khalid-jawed-profile.jpg" width="150" style="border-radius: 8px;" alt="M. Khalid Jawed"><br>
                <b>M. Khalid Jawed</b>
            </a>
            <p>
                数据驱动建模, 软体机器人, 流固耦合, 自动化, 计算力学, 仿生学
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/xiaochun-li/research-overview.md">
                <img src="research-quickview/ucla/profiles/xiaochun-li-profile.jpg" width="150" style="border-radius: 8px;" alt="Xiaochun Li"><br>
                <b>Xiaochun Li</b>
            </a>
            <p>
                科学驱动制造, 纳米颗粒分散超金属, 智能制造, 增材制造, 纳米制造, 工业应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/brett-lopez/research-overview.md">
                <img src="research-quickview/ucla/profiles/brett-lopez-profile.jpg" width="150" style="border-radius: 8px;" alt="Brett Lopez"><br>
                <b>Brett Lopez</b>
            </a>
            <p>
                航空航天机器人, 自主系统, 非线性控制, 轨迹规划, 本地化与建图, 智能系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/jacob-rosen/research-overview.md">
                <img src="research-quickview/ucla/profiles/jacob-rosen-profile.jpg" width="150" style="border-radius: 8px;" alt="Jacob Rosen"><br>
                <b>Jacob Rosen</b>
            </a>
            <p>
                医疗机器人, 外骨骼, 手术机器人, 生物力学, 虚拟现实, 人机交互与神经控制
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/veronica-santos/research-overview.md">
                <img src="research-quickview/ucla/profiles/veronica-santos-profile.jpg" width="150" style="border-radius: 8px;" alt="Veronica Santos"><br>
                <b>Veronica Santos</b>
            </a>
            <p>
                触觉传感, 假肢设计, 手部生物力学, 人机系统, 机器人力学, 随机建模
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/tsu-chin-tsao/research-overview.md">
                <img src="research-quickview/ucla/profiles/tsu-chin-tsao-profile.jpg" width="150" style="border-radius: 8px;" alt="Tsu-Chin Tsao"><br>
                <b>Tsu-Chin Tsao</b>
            </a>
            <p>
                动态系统建模, 精密控制, 机械系统控制, 数字控制, 自适应与优化控制, 机电一体化
            </p>
        </td>
    </tr>
</table>

- **Field**: [Fluid Mechanics](https://www.mae.ucla.edu/faculty-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/artur-davoyan/research-overview.md">
                <img src="research-quickview/ucla/profiles/artur-davoyan-profile.jpg" width="150" style="border-radius: 8px;" alt="Artur Davoyan"><br>
                <b>Artur Davoyan</b>
            </a>
            <p>
                空间、能源和通信中的先进材料与系统, 突破性推进技术, 高功率与高能量激光, 纳米材料与二维材料, 超材料, 等离子体光学, 光子器件
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/jeff-eldredge/research-overview.md">
                <img src="research-quickview/ucla/profiles/jeff-eldredge-profile.jpg" width="150" style="border-radius: 8px;" alt="Jeff Eldredge"><br>
                <b>Jeff Eldredge</b>
            </a>
            <p>
                流体动力学计算与理论研究, 包括非定常气动力学, 仿生运动, 流固耦合, 生物医学流体
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/ann-karagozian/research-overview.md">
                <img src="research-quickview/ucla/profiles/ann-karagozian-profile.jpg" width="150" style="border-radius: 8px;" alt="Ann Karagozian"><br>
                <b>Ann Karagozian</b>
            </a>
            <p>
                流体力学与燃烧, 改进发动机效率, 减少排放, 替代燃料, 高速空气动力与火箭推进
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/pirouz-kavehpour/research-overview.md">
                <img src="research-quickview/ucla/profiles/pirouz-kavehpour-profile.jpg" width="150" style="border-radius: 8px;" alt="Pirouz Kavehpour"><br>
                <b>Pirouz Kavehpour</b>
            </a>
            <p>
                微流体与生物流体, 生物燃料电池, 心血管流动, 复杂流体, 界面物理, 微型摩擦学, 非等温流, 药物输送与人工器官
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/mitchell-spearrin/research-overview.md">
                <img src="research-quickview/ucla/profiles/mitchell-spearrin-profile.jpg" width="150" style="border-radius: 8px;" alt="Mitchell Spearrin"><br>
                <b>Mitchell Spearrin</b>
            </a>
            <p>
                光谱学与气体动力学, 激光吸收与荧光传感器, 推进系统与能源系统实验应用, 反应流场研究
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/kunihiko-taira/research-overview.md">
                <img src="research-quickview/ucla/profiles/kunihiko-taira-profile.jpg" width="150" style="border-radius: 8px;" alt="Kunihiko Taira"><br>
                <b>Kunihiko Taira</b>
            </a>
            <p>
                流体流动的分析、建模与控制, 使用高性能计算, 流体力学、动力系统理论、网络科学与数据科学的结合
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/xiaolin-zhong/research-overview.md">
                <img src="research-quickview/ucla/profiles/xiaolin-zhong-profile.jpg" width="150" style="border-radius: 8px;" alt="Xiaolin Zhong"><br>
                <b>Xiaolin Zhong</b>
            </a>
            <p>
                计算流体动力学, 高超音速流, 高阶数值方法开发, 高超音速边界层感受性、稳定性与过渡, 湍流冲击波相互作用的直接数值模拟, 波能捕获装置的模拟研究
            </p>
        </td>
    </tr>
</table>

- **Field**: [Micro-Nano Engineering](https://www.mae.ucla.edu/faculty-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/greg-carman/research-overview.md">
                <img src="research-quickview/ucla/profiles/greg-carman-profile.jpg" width="150" style="border-radius: 8px;" alt="Greg Carman"><br>
                <b>Greg Carman</b>
            </a>
            <p>
                纳米级多铁材料, 压电材料, 磁致伸缩材料, 薄膜形状记忆合金, 光纤传感器, 智能材料
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/yong-chen/research-overview.md">
                <img src="research-quickview/ucla/profiles/yong-chen-profile.jpg" width="150" style="border-radius: 8px;" alt="Yong Chen"><br>
                <b>Yong Chen</b>
            </a>
            <p>
                纳米制造, 纳米电子材料与器件, 微纳电子/光学/生物/机械系统, 超空间和时间分辨表征
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/pei-yu-chiou/research-overview.md">
                <img src="research-quickview/ucla/profiles/pei-yu-chiou-profile.jpg" width="150" style="border-radius: 8px;" alt="Pei-Yu Chiou"><br>
                <b>Pei-Yu Chiou</b>
            </a>
            <p>
                生物光子学, 纳米光子学, BioMEMS/NEMS, 电动流体学, 微流体, 引导自组装, 高通量单细胞分析
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/artur-davoyan/research-overview.md">
                <img src="research-quickview/ucla/profiles/artur-davoyan-profile.jpg" width="150" style="border-radius: 8px;" alt="Artur Davoyan"><br>
                <b>Artur Davoyan</b>
            </a>
            <p>
                空间、能源和通信中的先进材料与系统, 高功率与高能激光, 激光材料相互作用, 纳米和二维材料, 超材料, 等离子体光学
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/tim-fisher/research-overview.md">
                <img src="research-quickview/ucla/profiles/tim-fisher-profile.jpg" width="150" style="border-radius: 8px;" alt="Tim Fisher"><br>
                <b>Tim Fisher</b>
            </a>
            <p>
                电子、声子、光子和流体的传输过程, 能源传输、转换和存储中的应用, 微纳电子、传感器工业
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/vijay-gupta/research-overview.md">
                <img src="research-quickview/ucla/profiles/vijay-gupta-profile.jpg" width="150" style="border-radius: 8px;" alt="Vijay Gupta"><br>
                <b>Vijay Gupta</b>
            </a>
            <p>
                实验力学, 工程固体的断裂, 薄膜和界面的力学, 复合材料的失效机理, 冰力学
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/yongjie-hu/research-overview.md">
                <img src="research-quickview/ucla/profiles/yongjie-hu-profile.jpg" width="150" style="border-radius: 8px;" alt="Yongjie Hu"><br>
                <b>Yongjie Hu</b>
            </a>
            <p>
                纳米结构中的热传导与电子传输, 热电设备, 能量转换与热管理, 超快光学光谱
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/lihua-jin/research-overview.md">
                <img src="research-quickview/ucla/profiles/lihua-jin-profile.jpg" width="150" style="border-radius: 8px;" alt="Lihua Jin"><br>
                <b>Lihua Jin</b>
            </a>
            <p>
                软材料力学, 不稳定性与断裂, 响应性材料, 连续力学, 添加剂制造, 软体机器人
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/sungtaek-ju/research-overview.md">
                <img src="research-quickview/ucla/profiles/sungtaek-ju-profile.jpg" width="150" style="border-radius: 8px;" alt="Y. Sungtaek Ju"><br>
                <b>Y. Sungtaek Ju</b>
            </a>
            <p>
                微纳尺度热科学, 能源, BioMEMS/NEMS, 纳米制造
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/pirouz-kavehpour/research-overview.md">
                <img src="research-quickview/ucla/profiles/pirouz-kavehpour-profile.jpg" width="150" style="border-radius: 8px;" alt="Pirouz Kavehpour"><br>
                <b>Pirouz Kavehpour</b>
            </a>
            <p>
                微流体与生物流体, 复杂流体, 界面物理, 微型摩擦学, 药物输送与人工器官
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/chang-jin-kim/research-overview.md">
                <img src="research-quickview/ucla/profiles/chang-jin-kim-profile.jpg" width="150" style="border-radius: 8px;" alt="Chang-Jin Kim"><br>
                <b>Chang-Jin Kim</b>
            </a>
            <p>
                微机电系统 (MEMS), 表面张力驱动微致动器, 纳米技术表面控制, 微流体设备, 微机加工技术
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/xiaochun-li/research-overview.md">
                <img src="research-quickview/ucla/profiles/xiaochun-li-profile.jpg" width="150" style="border-radius: 8px;" alt="Xiaochun Li"><br>
                <b>Xiaochun Li</b>
            </a>
            <p>
                科学驱动制造, 纳米颗粒分散超金属, 智能制造, 增材制造, 纳米制造, 工业应用
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/neil-lin/research-overview.md">
                <img src="research-quickview/ucla/profiles/neil-lin-profile.jpg" width="150" style="border-radius: 8px;" alt="Neil Lin"><br>
                <b>Neil Lin</b>
            </a>
            <p>
                3D打印组织, 模拟人类器官的几何结构与功能, 药物筛选复杂模型组织, 人造器官移植应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/laurent-pilon/research-overview.md">
                <img src="research-quickview/ucla/profiles/laurent-pilon-profile.jpg" width="150" style="border-radius: 8px;" alt="Laurent Pilon"><br>
                <b>Laurent Pilon</b>
            </a>
            <p>
                辐射传递, 应用光学, 界面与传输现象, 纳米结构材料在能源中的应用, 热电能源转换, 生物燃料光合生产
            </p>
        </td>
    </tr>
</table>

- **Field**: [Structural and Solid Mechanics](https://www.mae.ucla.edu/faculty-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/greg-carman/research-overview.md">
                <img src="research-quickview/ucla/profiles/greg-carman-profile.jpg" width="150" style="border-radius: 8px;" alt="Greg Carman"><br>
                <b>Greg Carman</b>
            </a>
            <p>
                纳米级多铁材料, 压电材料, 磁致伸缩材料, 薄膜形状记忆合金, 光纤传感器, 智能材料
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/artur-davoyan/research-overview.md">
                <img src="research-quickview/ucla/profiles/artur-davoyan-profile.jpg" width="150" style="border-radius: 8px;" alt="Artur Davoyan"><br>
                <b>Artur Davoyan</b>
            </a>
            <p>
                空间、能源和通信中的先进材料与系统, 高功率与高能量激光, 激光材料相互作用, 纳米和二维材料, 超材料, 等离子体光学
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/vijay-gupta/research-overview.md">
                <img src="research-quickview/ucla/profiles/vijay-gupta-profile.jpg" width="150" style="border-radius: 8px;" alt="Vijay Gupta"><br>
                <b>Vijay Gupta</b>
            </a>
            <p>
                实验力学, 工程固体的断裂, 薄膜和界面的力学, 复合材料的失效机理, 冰力学, 高频声学脉冲应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/jonathan-hopkins/research-overview.md">
                <img src="research-quickview/ucla/profiles/jonathan-hopkins-profile.jpg" width="150" style="border-radius: 8px;" alt="Jonathan B. Hopkins"><br>
                <b>Jonathan B. Hopkins</b>
            </a>
            <p>
                柔性结构的设计与分析, 复杂机制的制造, 精密结构, 创新结构的机械性能
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/m-khalid-jawed/research-overview.md">
                <img src="research-quickview/ucla/profiles/m-khalid-jawed-profile.jpg" width="150" style="border-radius: 8px;" alt="M. Khalid Jawed"><br>
                <b>M. Khalid Jawed</b>
            </a>
            <p>
                数据驱动结构力学建模, 流固耦合, 机器人与自动化, 计算与机器学习在力学中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/lihua-jin/research-overview.md">
                <img src="research-quickview/ucla/profiles/lihua-jin-profile.jpg" width="150" style="border-radius: 8px;" alt="Lihua Jin"><br>
                <b>Lihua Jin</b>
            </a>
            <p>
                软材料力学, 材料不稳定性与断裂, 响应性材料, 添加剂制造, 软体机器人, 可拉伸电子产品, 生物力学
            </p>
        </td>
    </tr>
</table>

- **Field**: [Systems and Control](https://www.mae.ucla.edu/faculty-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/anushri-dixit/research-overview.md">
                <img src="research-quickview/ucla/profiles/anushri-dixit-profile.jpg" width="150" style="border-radius: 8px;" alt="Anushri Dixit"><br>
                <b>Anushri Dixit</b>
            </a>
            <p>
                自主性, 机器人学, 安全关键决策, 随机控制, 模型预测控制, 风险感知规划
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/elisa-franco/research-overview.md">
                <img src="research-quickview/ucla/profiles/elisa-franco-profile.jpg" width="150" style="border-radius: 8px;" alt="Elisa Franco"><br>
                <b>Elisa Franco</b>
            </a>
            <p>
                结构生物学, 动态与控制, 生物分子框架, 合成生物学, 智能材料
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/jonathan-hopkins/research-overview.md">
                <img src="research-quickview/ucla/profiles/jonathan-hopkins-profile.jpg" width="150" style="border-radius: 8px;" alt="Jonathan B. Hopkins"><br>
                <b>Jonathan B. Hopkins</b>
            </a>
            <p>
                柔性结构的设计与分析, 复杂机制的制造, 精密结构, 创新结构的机械性能
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/tetsuya-iwasaki/research-overview.md">
                <img src="research-quickview/ucla/profiles/tetsuya-iwasaki-profile.jpg" width="150" style="border-radius: 8px;" alt="Tetsuya Iwasaki"><br>
                <b>Tetsuya Iwasaki</b>
            </a>
            <p>
                动物运动的神经控制机制, 非线性振荡器, 鲁棒与非线性控制理论及其机械、航空航天和电气系统的应用
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/brett-lopez/research-overview.md">
                <img src="research-quickview/ucla/profiles/brett-lopez-profile.jpg" width="150" style="border-radius: 8px;" alt="Brett Lopez"><br>
                <b>Brett Lopez</b>
            </a>
            <p>
                自主系统, 航空航天机器人, 非线性与自适应控制, 预测控制, 轨迹规划, 本地化与建图
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/robert-mcloskey/research-overview.md">
                <img src="research-quickview/ucla/profiles/robert-mcloskey-profile.jpg" width="150" style="border-radius: 8px;" alt="Robert M’Closkey"><br>
                <b>Robert M’Closkey</b>
            </a>
            <p>
                非线性控制理论与设计, 机械与航空航天系统的实时实施, 动态控制系统优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/jacob-rosen/research-overview.md">
                <img src="research-quickview/ucla/profiles/jacob-rosen-profile.jpg" width="150" style="border-radius: 8px;" alt="Jacob Rosen"><br>
                <b>Jacob Rosen</b>
            </a>
            <p>
                医疗机器人, 人机交互, 手术机器人, 外骨骼机器人, 神经控制, 生物力学, 虚拟现实与触觉技术
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/veronica-santos/research-overview.md">
                <img src="research-quickview/ucla/profiles/veronica-santos-profile.jpg" width="150" style="border-radius: 8px;" alt="Veronica Santos"><br>
                <b>Veronica Santos</b>
            </a>
            <p>
                人机系统, 机器人假肢, 触觉传感器, 手部生物力学, 神经运动控制, 随机建模
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/jason-speyer/research-overview.md">
                <img src="research-quickview/ucla/profiles/jason-speyer-profile.jpg" width="150" style="border-radius: 8px;" alt="Jason L. Speyer"><br>
                <b>Jason L. Speyer</b>
            </a>
            <p>
                随机与确定性最优控制与估计, 航空航天系统的导航与控制, 飞行力学, 指南与优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/tsu-chin-tsao/research-overview.md">
                <img src="research-quickview/ucla/profiles/tsu-chin-tsao-profile.jpg" width="150" style="border-radius: 8px;" alt="Tsu-Chin Tsao"><br>
                <b>Tsu-Chin Tsao</b>
            </a>
            <p>
                动态系统建模与控制, 机械系统的数字与重复控制, 学习控制, 自适应与最优控制, 机电一体化
            </p>
        </td>
    </tr>
</table>

- **Field**: [Thermal Science and Engineering](https://www.mae.ucla.edu/faculty-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/vijay-dhir/research-overview.md">
                <img src="research-quickview/ucla/profiles/vijay-dhir-profile.jpg" width="150" style="border-radius: 8px;" alt="Vijay Dhir"><br>
                <b>Vijay Dhir</b>
            </a>
            <p>
                两相传热, 沸腾与冷凝, 热稳定性与流体稳定性, 核反应堆的热水力学, 微重力传热, 土壤修复
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/tim-fisher/research-overview.md">
                <img src="research-quickview/ucla/profiles/tim-fisher-profile.jpg" width="150" style="border-radius: 8px;" alt="Tim Fisher"><br>
                <b>Tim Fisher</b>
            </a>
            <p>
                电子、声子、光子和流体的传输过程, 能源传输、转换与存储的工业应用, 包括航空航天、微纳电子与传感器
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/yongjie-hu/research-overview.md">
                <img src="research-quickview/ucla/profiles/yongjie-hu-profile.jpg" width="150" style="border-radius: 8px;" alt="Yongjie Hu"><br>
                <b>Yongjie Hu</b>
            </a>
            <p>
                纳米结构中的热传递与电子传输, 热电设备与系统, 能量转换与热管理, 超快光学光谱与高频电子, 纳米材料设计与制造
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/sungtaek-ju/research-overview.md">
                <img src="research-quickview/ucla/profiles/sungtaek-ju-profile.jpg" width="150" style="border-radius: 8px;" alt="Y. Sungtaek Ju"><br>
                <b>Y. Sungtaek Ju</b>
            </a>
            <p>
                微纳热科学, 能源, BioMEMS/NEMS, 纳米制造
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/mae/pirouz-kavehpour/research-overview.md">
                <img src="research-quickview/ucla/profiles/pirouz-kavehpour-profile.jpg" width="150" style="border-radius: 8px;" alt="Pirouz Kavehpour"><br>
                <b>Pirouz Kavehpour</b>
            </a>
            <p>
                微流体与生物流体, 生物燃料电池, 心血管流动, 复杂流体, 界面物理, 药物输送与人工器官
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/adrienne-lavine/research-overview.md">
                <img src="research-quickview/ucla/profiles/adrienne-lavine-profile.jpg" width="150" style="border-radius: 8px;" alt="Adrienne Lavine"><br>
                <b>Adrienne Lavine</b>
            </a>
            <p>
                太阳能热储存, 热能收集, 纳米制造的热控制, 形状记忆合金的热机械行为, 加工过程的热效应
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/laurent-pilon/research-overview.md">
                <img src="research-quickview/ucla/profiles/laurent-pilon-profile.jpg" width="150" style="border-radius: 8px;" alt="Laurent Pilon"><br>
                <b>Laurent Pilon</b>
            </a>
            <p>
                辐射传递, 应用光学, 界面与传输现象, 纳米结构材料在能源中的应用, 热电能源转换, 生物燃料光合生产
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/mae/mitchell-spearrin/research-overview.md">
                <img src="research-quickview/ucla/profiles/mitchell-spearrin-profile.jpg" width="150" style="border-radius: 8px;" alt="Mitchell Spearrin"><br>
                <b>Mitchell Spearrin</b>
            </a>
            <p>
                光谱学与气体动力学, 高级光学传感器, 包括激光吸收与荧光, 推进与能源系统实验应用, 反应流场研究
            </p>
        </td>
    </tr>
</table>



### [Electrical & Computer Engineering](https://www.ee.ucla.edu/ucla-ee-research/)

- **Field**: [Artificial Intelligence, Data Science, and Machine Learning](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/abeer-alwan/research-overview.md">
                <img src="research-quickview/ucla/profiles/abeer-alwan-profile.jpg" width="150" style="border-radius: 8px;" alt="Abeer Alwan"><br>
                <b>Abeer Alwan</b>
            </a>
            <p>
                语音信号处理, 语音识别, 语音合成, 机器学习在语音技术中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/anthony-chen/research-overview.md">
                <img src="research-quickview/ucla/profiles/anthony-chen-profile.jpg" width="150" style="border-radius: 8px;" alt="Anthony Chen"><br>
                <b>Anthony Chen</b>
            </a>
            <p>
                人工智能, 数据科学, 深度学习, 计算智能, 自动化与优化算法
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/suhas-diggavi/research-overview.md">
                <img src="research-quickview/ucla/profiles/suhas-diggavi-profile.jpg" width="150" style="border-radius: 8px;" alt="Suhas Diggavi"><br>
                <b>Suhas Diggavi</b>
            </a>
            <p>
                信息理论, 机器学习, 数据隐私, 网络与分布式系统, 安全性与加密
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lara-dolecek/research-overview.md">
                <img src="research-quickview/ucla/profiles/lara-dolecek-profile.jpg" width="150" style="border-radius: 8px;" alt="Lara Dolecek"><br>
                <b>Lara Dolecek</b>
            </a>
            <p>
                编码理论, 信息处理, 数据存储系统, 统计学习, 通信系统的优化
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/sam-emamjinejad/research-overview.md">
                <img src="research-quickview/ucla/profiles/sam-emamjinejad-profile.jpg" width="150" style="border-radius: 8px;" alt="Sam Emamjinejad"><br>
                <b>Sam Emamjinejad</b>
            </a>
            <p>
                生物医学信号处理, 机器学习在医疗中的应用, 数据驱动健康分析
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/christina-fragouli/research-overview.md">
                <img src="research-quickview/ucla/profiles/christina-fragouli-profile.jpg" width="150" style="border-radius: 8px;" alt="Christina Fragouli"><br>
                <b>Christina Fragouli</b>
            </a>
            <p>
                网络编码, 数据科学, 信息流优化, 机器学习在网络中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/puneet-gupta/research-overview.md">
                <img src="research-quickview/ucla/profiles/puneet-gupta-profile.jpg" width="150" style="border-radius: 8px;" alt="Puneet Gupta"><br>
                <b>Puneet Gupta</b>
            </a>
            <p>
                智能制造, 设计优化, 半导体技术, 大数据分析, 智能系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lei-he/research-overview.md">
                <img src="research-quickview/ucla/profiles/lei-he-profile.jpg" width="150" style="border-radius: 8px;" alt="Lei He"><br>
                <b>Lei He</b>
            </a>
            <p>
                数据驱动设计, 嵌入式系统, 集成电路优化, 深度学习与硬件协同设计
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/achuta-kadambi/research-overview.md">
                <img src="research-quickview/ucla/profiles/achuta-kadambi-profile.jpg" width="150" style="border-radius: 8px;" alt="Achuta Kadambi"><br>
                <b>Achuta Kadambi</b>
            </a>
            <p>
                计算成像, 人工智能驱动成像技术, 机器学习在视觉系统中的应用, 光学传感器
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/jonathan-kao/research-overview.md">
                <img src="research-quickview/ucla/profiles/jonathan-kao-profile.jpg" width="150" style="border-radius: 8px;" alt="Jonathan Kao"><br>
                <b>Jonathan Kao</b>
            </a>
            <p>
                神经网络, 脑机接口, 生物信号处理, 机器学习在神经科学中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/di-luo/research-overview.md">
                <img src="research-quickview/ucla/profiles/di-luo-profile.jpg" width="150" style="border-radius: 8px;" alt="Di Luo"><br>
                <b>Di Luo</b>
            </a>
            <p>
                深度学习, 数据分析, 强化学习, 人工智能在自动化中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/aydogan-ozcan/research-overview.md">
                <img src="research-quickview/ucla/profiles/aydogan-ozcan-profile.jpg" width="150" style="border-radius: 8px;" alt="Aydogan Ozcan"><br>
                <b>Aydogan Ozcan</b>
            </a>
            <p>
                数字病理学, 人工智能驱动成像, 纳米技术, 光学计算
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/sudhakar-pamarti/research-overview.md">
                <img src="research-quickview/ucla/profiles/sudhakar-pamarti-profile.jpg" width="150" style="border-radius: 8px;" alt="Sudhakar Pamarti"><br>
                <b>Sudhakar Pamarti</b>
            </a>
            <p>
                通信系统, 模拟与混合信号电路, 数据转换器, 机器学习在信号处理中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/vwani-roychowdhury/research-overview.md">
                <img src="research-quickview/ucla/profiles/vwani-roychowdhury-profile.jpg" width="150" style="border-radius: 8px;" alt="Vwani P. Roychowdhury"><br>
                <b>Vwani P. Roychowdhury</b>
            </a>
            <p>
                量子计算, 机器学习算法, 网络科学, 数据科学与人工智能
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/nader-sehatbakhsh/research-overview.md">
                <img src="research-quickview/ucla/profiles/nader-sehatbakhsh-profile.jpg" width="150" style="border-radius: 8px;" alt="Nader Sehatbakhsh"><br>
                <b>Nader Sehatbakhsh</b>
            </a>
            <p>
                深度学习硬件加速, 神经网络优化, 嵌入式系统, 高效计算架构
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/mani-srivastava/research-overview.md">
                <img src="research-quickview/ucla/profiles/mani-srivastava-profile.jpg" width="150" style="border-radius: 8px;" alt="Mani Srivastava"><br>
                <b>Mani Srivastava</b>
            </a>
            <p>
                人工智能物联网, 传感器网络, 嵌入式AI系统, 数据科学与优化
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/paulo-tabuada/research-overview.md">
                <img src="research-quickview/ucla/profiles/paulo-tabuada-profile.jpg" width="150" style="border-radius: 8px;" alt="Paulo Tabuada"><br>
                <b>Paulo Tabuada</b>
            </a>
            <p>
                控制理论, 网络系统, 自动化系统, 人工智能与数据优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/yuan-tian/research-overview.md">
                <img src="research-quickview/ucla/profiles/yuan-tian-profile.jpg" width="150" style="border-radius: 8px;" alt="Yuan Tian"><br>
                <b>Yuan Tian</b>
            </a>
            <p>
                机器学习安全性, 数据隐私, 网络安全, 人工智能与伦理
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lieven-vandenberghe/research-overview.md">
                <img src="research-quickview/ucla/profiles/lieven-vandenberghe-profile.jpg" width="150" style="border-radius: 8px;" alt="Lieven Vandenberghe"><br>
                <b>Lieven Vandenberghe</b>
            </a>
            <p>
                优化算法, 机器学习, 数据分析, 控制系统优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/john-villasenor/research-overview.md">
                <img src="research-quickview/ucla/profiles/john-villasenor-profile.jpg" width="150" style="border-radius: 8px;" alt="John Villasenor"><br>
                <b>John Villasenor</b>
            </a>
            <p>
                数据科学, 人工智能政策, 机器学习, 网络与信息安全
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/ck-yang/research-overview.md">
                <img src="research-quickview/ucla/profiles/ck-yang-profile.jpg" width="150" style="border-radius: 8px;" alt="C. K. 'Ken' Yang"><br>
                <b>C. K. 'Ken' Yang</b>
            </a>
            <p>
                集成电路设计, 数字与模拟电路, 机器学习硬件加速
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lin-yang/research-overview.md">
                <img src="research-quickview/ucla/profiles/lin-yang-profile.jpg" width="150" style="border-radius: 8px;" alt="Lin Yang"><br>
                <b>Lin Yang</b>
            </a>
            <p>
                强化学习, 数据科学, 高效优化算法, 机器学习在控制系统中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/yang-zhang/research-overview.md">
                <img src="research-quickview/ucla/profiles/yang-zhang-profile.jpg" width="150" style="border-radius: 8px;" alt="Yang Zhang"><br>
                <b>Yang Zhang</b>
            </a>
            <p>
                人工智能安全, 网络科学, 机器学习, 数据驱动系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/leo-zhou/research-overview.md">
                <img src="research-quickview/ucla/profiles/leo-zhou-profile.jpg" width="150" style="border-radius: 8px;" alt="Leo Zhou"><br>
                <b>Leo Zhou</b>
            </a>
            <p>
                量子计算, 人工智能优化, 深度学习, 数据科学与建模
            </p>
        </td>
    </tr>
</table>

- **Field**: [Human-Computer Interface, Mobile Computing and IoT](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/abeer-alwan/research-overview.md">
                <img src="research-quickview/ucla/profiles/abeer-alwan-profile.jpg" width="150" style="border-radius: 8px;" alt="Abeer Alwan"><br>
                <b>Abeer Alwan</b>
            </a>
            <p>
                语音信号处理, 语音识别, 语音合成, 机器学习在语音技术中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/anthony-chen/research-overview.md">
                <img src="research-quickview/ucla/profiles/anthony-chen-profile.jpg" width="150" style="border-radius: 8px;" alt="Anthony Chen"><br>
                <b>Anthony Chen</b>
            </a>
            <p>
                人工智能, 数据科学, 深度学习, 计算智能, 自动化与优化算法
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/suhas-diggavi/research-overview.md">
                <img src="research-quickview/ucla/profiles/suhas-diggavi-profile.jpg" width="150" style="border-radius: 8px;" alt="Suhas Diggavi"><br>
                <b>Suhas Diggavi</b>
            </a>
            <p>
                信息理论, 机器学习, 数据隐私, 网络与分布式系统, 安全性与加密
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lara-dolecek/research-overview.md">
                <img src="research-quickview/ucla/profiles/lara-dolecek-profile.jpg" width="150" style="border-radius: 8px;" alt="Lara Dolecek"><br>
                <b>Lara Dolecek</b>
            </a>
            <p>
                编码理论, 信息处理, 数据存储系统, 统计学习, 通信系统的优化
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/sam-emamjinejad/research-overview.md">
                <img src="research-quickview/ucla/profiles/sam-emamjinejad-profile.jpg" width="150" style="border-radius: 8px;" alt="Sam Emamjinejad"><br>
                <b>Sam Emamjinejad</b>
            </a>
            <p>
                生物医学信号处理, 机器学习在医疗中的应用, 数据驱动健康分析
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/christina-fragouli/research-overview.md">
                <img src="research-quickview/ucla/profiles/christina-fragouli-profile.jpg" width="150" style="border-radius: 8px;" alt="Christina Fragouli"><br>
                <b>Christina Fragouli</b>
            </a>
            <p>
                网络编码, 数据科学, 信息流优化, 机器学习在网络中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/puneet-gupta/research-overview.md">
                <img src="research-quickview/ucla/profiles/puneet-gupta-profile.jpg" width="150" style="border-radius: 8px;" alt="Puneet Gupta"><br>
                <b>Puneet Gupta</b>
            </a>
            <p>
                智能制造, 设计优化, 半导体技术, 大数据分析, 智能系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lei-he/research-overview.md">
                <img src="research-quickview/ucla/profiles/lei-he-profile.jpg" width="150" style="border-radius: 8px;" alt="Lei He"><br>
                <b>Lei He</b>
            </a>
            <p>
                数据驱动设计, 嵌入式系统, 集成电路优化, 深度学习与硬件协同设计
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/achuta-kadambi/research-overview.md">
                <img src="research-quickview/ucla/profiles/achuta-kadambi-profile.jpg" width="150" style="border-radius: 8px;" alt="Achuta Kadambi"><br>
                <b>Achuta Kadambi</b>
            </a>
            <p>
                计算成像, 人工智能驱动成像技术, 机器学习在视觉系统中的应用, 光学传感器
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/jonathan-kao/research-overview.md">
                <img src="research-quickview/ucla/profiles/jonathan-kao-profile.jpg" width="150" style="border-radius: 8px;" alt="Jonathan Kao"><br>
                <b>Jonathan Kao</b>
            </a>
            <p>
                神经网络, 脑机接口, 生物信号处理, 机器学习在神经科学中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/di-luo/research-overview.md">
                <img src="research-quickview/ucla/profiles/di-luo-profile.jpg" width="150" style="border-radius: 8px;" alt="Di Luo"><br>
                <b>Di Luo</b>
            </a>
            <p>
                深度学习, 数据分析, 强化学习, 人工智能在自动化中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/aydogan-ozcan/research-overview.md">
                <img src="research-quickview/ucla/profiles/aydogan-ozcan-profile.jpg" width="150" style="border-radius: 8px;" alt="Aydogan Ozcan"><br>
                <b>Aydogan Ozcan</b>
            </a>
            <p>
                数字病理学, 人工智能驱动成像, 纳米技术, 光学计算
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/sudhakar-pamarti/research-overview.md">
                <img src="research-quickview/ucla/profiles/sudhakar-pamarti-profile.jpg" width="150" style="border-radius: 8px;" alt="Sudhakar Pamarti"><br>
                <b>Sudhakar Pamarti</b>
            </a>
            <p>
                通信系统, 模拟与混合信号电路, 数据转换器, 机器学习在信号处理中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/vwani-roychowdhury/research-overview.md">
                <img src="research-quickview/ucla/profiles/vwani-roychowdhury-profile.jpg" width="150" style="border-radius: 8px;" alt="Vwani P. Roychowdhury"><br>
                <b>Vwani P. Roychowdhury</b>
            </a>
            <p>
                量子计算, 机器学习算法, 网络科学, 数据科学与人工智能
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/nader-sehatbakhsh/research-overview.md">
                <img src="research-quickview/ucla/profiles/nader-sehatbakhsh-profile.jpg" width="150" style="border-radius: 8px;" alt="Nader Sehatbakhsh"><br>
                <b>Nader Sehatbakhsh</b>
            </a>
            <p>
                深度学习硬件加速, 神经网络优化, 嵌入式系统, 高效计算架构
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/mani-srivastava/research-overview.md">
                <img src="research-quickview/ucla/profiles/mani-srivastava-profile.jpg" width="150" style="border-radius: 8px;" alt="Mani Srivastava"><br>
                <b>Mani Srivastava</b>
            </a>
            <p>
                人工智能物联网, 传感器网络, 嵌入式AI系统, 数据科学与优化
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/paulo-tabuada/research-overview.md">
                <img src="research-quickview/ucla/profiles/paulo-tabuada-profile.jpg" width="150" style="border-radius: 8px;" alt="Paulo Tabuada"><br>
                <b>Paulo Tabuada</b>
            </a>
            <p>
                控制理论, 网络系统, 自动化系统, 人工智能与数据优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/yuan-tian/research-overview.md">
                <img src="research-quickview/ucla/profiles/yuan-tian-profile.jpg" width="150" style="border-radius: 8px;" alt="Yuan Tian"><br>
                <b>Yuan Tian</b>
            </a>
            <p>
                机器学习安全性, 数据隐私, 网络安全, 人工智能与伦理
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lieven-vandenberghe/research-overview.md">
                <img src="research-quickview/ucla/profiles/lieven-vandenberghe-profile.jpg" width="150" style="border-radius: 8px;" alt="Lieven Vandenberghe"><br>
                <b>Lieven Vandenberghe</b>
            </a>
            <p>
                优化算法, 机器学习, 数据分析, 控制系统优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/john-villasenor/research-overview.md">
                <img src="research-quickview/ucla/profiles/john-villasenor-profile.jpg" width="150" style="border-radius: 8px;" alt="John Villasenor"><br>
                <b>John Villasenor</b>
            </a>
            <p>
                数据科学, 人工智能政策, 机器学习, 网络与信息安全
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/ck-yang/research-overview.md">
                <img src="research-quickview/ucla/profiles/ck-yang-profile.jpg" width="150" style="border-radius: 8px;" alt="C. K. 'Ken' Yang"><br>
                <b>C. K. 'Ken' Yang</b>
            </a>
            <p>
                集成电路设计, 数字与模拟电路, 机器学习硬件加速
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lin-yang/research-overview.md">
                <img src="research-quickview/ucla/profiles/lin-yang-profile.jpg" width="150" style="border-radius: 8px;" alt="Lin Yang"><br>
                <b>Lin Yang</b>
            </a>
            <p>
                强化学习, 数据科学, 高效优化算法, 机器学习在控制系统中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/yang-zhang/research-overview.md">
                <img src="research-quickview/ucla/profiles/yang-zhang-profile.jpg" width="150" style="border-radius: 8px;" alt="Yang Zhang"><br>
                <b>Yang Zhang</b>
            </a>
            <p>
                人工智能安全, 网络科学, 机器学习, 数据驱动系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/leo-zhou/research-overview.md">
                <img src="research-quickview/ucla/profiles/leo-zhou-profile.jpg" width="150" style="border-radius: 8px;" alt="Leo Zhou"><br>
                <b>Leo Zhou</b>
            </a>
            <p>
                量子计算, 人工智能优化, 深度学习, 数据科学与建模
            </p>
        </td>
    </tr>
</table>

- **Field**: [Computing Architectures and Design Automation](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/frank-chang/research-overview.md">
                <img src="research-quickview/ucla/profiles/frank-chang-profile.jpg" width="150" style="border-radius: 8px;" alt="Frank Chang"><br>
                <b>Frank Chang</b>
            </a>
            <p>
                计算架构设计, 高速无线通信, 半导体技术, 微波与毫米波系统, 集成电路优化, 高性能计算系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/puneet-gupta/research-overview.md">
                <img src="research-quickview/ucla/profiles/puneet-gupta-profile.jpg" width="150" style="border-radius: 8px;" alt="Puneet Gupta"><br>
                <b>Puneet Gupta</b>
            </a>
            <p>
                设计自动化, 智能制造, 半导体优化, 纳米级集成电路设计, 数据驱动系统开发, 硬件与软件协同优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lei-he/research-overview.md">
                <img src="research-quickview/ucla/profiles/lei-he-profile.jpg" width="150" style="border-radius: 8px;" alt="Lei He"><br>
                <b>Lei He</b>
            </a>
            <p>
                嵌入式系统设计, 数据驱动优化, 集成电路设计与自动化, 深度学习与硬件协同设计, 低功耗计算架构
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/di-luo/research-overview.md">
                <img src="research-quickview/ucla/profiles/di-luo-profile.jpg" width="150" style="border-radius: 8px;" alt="Di Luo"><br>
                <b>Di Luo</b>
            </a>
            <p>
                深度学习加速器设计, 数据分析, 强化学习在硬件中的应用, 硬件优化与自动化
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/dejan-markovic/research-overview.md">
                <img src="research-quickview/ucla/profiles/dejan-markovic-profile.jpg" width="150" style="border-radius: 8px;" alt="Dejan Markovic"><br>
                <b>Dejan Markovic</b>
            </a>
            <p>
                高性能计算架构, 神经网络硬件加速, 可编程逻辑设备, 低功耗VLSI设计, 嵌入式系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/nader-sehatbakhsh/research-overview.md">
                <img src="research-quickview/ucla/profiles/nader-sehatbakhsh-profile.jpg" width="150" style="border-radius: 8px;" alt="Nader Sehatbakhsh"><br>
                <b>Nader Sehatbakhsh</b>
            </a>
            <p>
                硬件加速器设计, 深度学习硬件优化, 嵌入式系统开发, 数据驱动高效计算, 低功耗架构
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/john-villasenor/research-overview.md">
                <img src="research-quickview/ucla/profiles/john-villasenor-profile.jpg" width="150" style="border-radius: 8px;" alt="John Villasenor"><br>
                <b>John Villasenor</b>
            </a>
            <p>
                数据科学, 计算架构政策, 硬件与软件协同优化, 网络安全, 高效计算系统, 人工智能与硬件
            </p>
        </td>
    </tr>
</table>

- **Field**: [Security and Trustworthy Systems](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/suhas-diggavi/research-overview.md">
                <img src="research-quickview/ucla/profiles/suhas-diggavi-profile.jpg" width="150" style="border-radius: 8px;" alt="Suhas Diggavi"><br>
                <b>Suhas Diggavi</b>
            </a>
            <p>
                信息理论, 数据隐私, 网络安全, 分布式系统, 机器学习中的安全性与可靠性
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/christina-fragouli/research-overview.md">
                <img src="research-quickview/ucla/profiles/christina-fragouli-profile.jpg" width="150" style="border-radius: 8px;" alt="Christina Fragouli"><br>
                <b>Christina Fragouli</b>
            </a>
            <p>
                网络编码, 数据安全, 信息流优化, 可信网络设计, 数据驱动通信优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/liz-izhikevich/research-overview.md">
                <img src="research-quickview/ucla/profiles/liz-izhikevich-profile.jpg" width="150" style="border-radius: 8px;" alt="Liz Izhikevich"><br>
                <b>Liz Izhikevich</b>
            </a>
            <p>
                网络安全, 隐私保护技术, 数据加密, 安全协议设计, 智能系统中的信任与验证
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/vwani-roychowdhury/research-overview.md">
                <img src="research-quickview/ucla/profiles/vwani-roychowdhury-profile.jpg" width="150" style="border-radius: 8px;" alt="Vwani P. Roychowdhury"><br>
                <b>Vwani P. Roychowdhury</b>
            </a>
            <p>
                数据隐私, 量子计算中的安全性, 网络科学, 人工智能与安全系统, 社会网络安全
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/nader-sehatbakhsh/research-overview.md">
                <img src="research-quickview/ucla/profiles/nader-sehatbakhsh-profile.jpg" width="150" style="border-radius: 8px;" alt="Nader Sehatbakhsh"><br>
                <b>Nader Sehatbakhsh</b>
            </a>
            <p>
                硬件安全, 深度学习加速器的安全性, 嵌入式系统保护, 数据加密优化, 可信计算架构
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/mani-srivastava/research-overview.md">
                <img src="research-quickview/ucla/profiles/mani-srivastava-profile.jpg" width="150" style="border-radius: 8px;" alt="Mani Srivastava"><br>
                <b>Mani Srivastava</b>
            </a>
            <p>
                物联网安全, 数据隐私与优化, 嵌入式系统保护, 可信计算, 网络安全与分布式系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/paulo-tabuada/research-overview.md">
                <img src="research-quickview/ucla/profiles/paulo-tabuada-profile.jpg" width="150" style="border-radius: 8px;" alt="Paulo Tabuada"><br>
                <b>Paulo Tabuada</b>
            </a>
            <p>
                网络系统安全, 控制系统中的数据隐私, 自动化与安全性, 分布式系统中的信任管理
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/yuan-tian/research-overview.md">
                <img src="research-quickview/ucla/profiles/yuan-tian-profile.jpg" width="150" style="border-radius: 8px;" alt="Yuan Tian"><br>
                <b>Yuan Tian</b>
            </a>
            <p>
                数据隐私, 人工智能系统的安全性, 网络安全, 智能家居系统保护, 隐私增强技术, 信任与验证
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/john-villasenor/research-overview.md">
                <img src="research-quickview/ucla/profiles/john-villasenor-profile.jpg" width="150" style="border-radius: 8px;" alt="John Villasenor"><br>
                <b>John Villasenor</b>
            </a>
            <p>
                网络安全政策, 数据保护, 数字化信任, 人工智能安全, 智能设备中的数据隐私, 可信系统设计
            </p>
        </td>
    </tr>
</table>

- **Field**: [Power and Energy Storage Systems](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/xiaofan-cui/research-overview.md">
                <img src="research-quickview/ucla/profiles/xiaofan-cui-profile.jpg" width="150" style="border-radius: 8px;" alt="Xiaofan Cui"><br>
                <b>Xiaofan Cui</b>
            </a>
            <p>
                电力系统优化, 能源存储系统, 智能电网技术, 可再生能源集成, 数据驱动能源管理, 电力电子
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/sam-emamjinejad/research-overview.md">
                <img src="research-quickview/ucla/profiles/sam-emamjinejad-profile.jpg" width="150" style="border-radius: 8px;" alt="Sam Emamjinejad"><br>
                <b>Sam Emamjinejad</b>
            </a>
            <p>
                智能能源监测, 数据驱动电力管理, 机器学习在能源系统中的应用, 可穿戴能源设备, 生物医学能源应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/subramanian-iyer/research-overview.md">
                <img src="research-quickview/ucla/profiles/subramanian-iyer-profile.jpg" width="150" style="border-radius: 8px;" alt="Subramanian Iyer"><br>
                <b>Subramanian Iyer</b>
            </a>
            <p>
                集成能源系统设计, 功率优化, 高效存储器与能源集成, 可持续能源硬件开发, 纳米级能源解决方案
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/chandrashekhar-joshi/research-overview.md">
                <img src="research-quickview/ucla/profiles/chandrashekhar-joshi-profile.jpg" width="150" style="border-radius: 8px;" alt="Chandrashekhar Joshi"><br>
                <b>Chandrashekhar Joshi</b>
            </a>
            <p>
                高能量密度存储系统, 等离子体能源转换, 高效能源材料开发, 先进能源存储系统
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/paulo-tabuada/research-overview.md">
                <img src="research-quickview/ucla/profiles/paulo-tabuada-profile.jpg" width="150" style="border-radius: 8px;" alt="Paulo Tabuada"><br>
                <b>Paulo Tabuada</b>
            </a>
            <p>
                能源系统的控制与优化, 智能电网, 分布式能源管理, 数据驱动能源效率, 电力调度与优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/kang-wang/research-overview.md">
                <img src="research-quickview/ucla/profiles/kang-wang-profile.jpg" width="150" style="border-radius: 8px;" alt="Kang L. Wang"><br>
                <b>Kang L. Wang</b>
            </a>
            <p>
                磁性存储系统, 低功耗能源设备, 纳米能源存储技术, 磁性能源材料, 可持续能源硬件
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/jason-woo/research-overview.md">
                <img src="research-quickview/ucla/profiles/jason-woo-profile.jpg" width="150" style="border-radius: 8px;" alt="Jason Woo"><br>
                <b>Jason Woo</b>
            </a>
            <p>
                高效能源集成, 半导体技术在能源中的应用, 能源硬件开发, 智能存储设备, 电力传输优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/ck-yang/research-overview.md">
                <img src="research-quickview/ucla/profiles/ck-yang-profile.jpg" width="150" style="border-radius: 8px;" alt="C. K. 'Ken' Yang"><br>
                <b>C. K. 'Ken' Yang</b>
            </a>
            <p>
                电力系统设计, 智能电力电子, 能源管理硬件, 集成电路在能源中的应用, 高效能源转换
            </p>
        </td>
    </tr>
</table>

- **Field**: [Integrated Circuits – Analog/Digital/Mixed-Signal](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/asad-abidi/research-overview.md">
                <img src="research-quickview/ucla/profiles/asad-abidi-profile.jpg" width="150" style="border-radius: 8px;" alt="Asad Abidi"><br>
                <b>Asad Abidi</b>
            </a>
            <p>
                模拟与混合信号集成电路, 射频电路设计, 高速数据转换器, 通信与信号处理硬件, 低功耗设计
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/aydin-babakhani/research-overview.md">
                <img src="research-quickview/ucla/profiles/aydin-babakhani-profile.jpg" width="150" style="border-radius: 8px;" alt="Aydin Babakhani"><br>
                <b>Aydin Babakhani</b>
            </a>
            <p>
                毫米波与太赫兹集成电路, 无线通信系统, 射频电路设计, 超高速数据传输, 低功耗射频传感器
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/frank-chang/research-overview.md">
                <img src="research-quickview/ucla/profiles/frank-chang-profile.jpg" width="150" style="border-radius: 8px;" alt="Frank Chang"><br>
                <b>Frank Chang</b>
            </a>
            <p>
                集成电路设计, 高速无线通信, 半导体技术, 微波与毫米波系统, 高性能计算系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/sam-emamjinejad/research-overview.md">
                <img src="research-quickview/ucla/profiles/sam-emamjinejad-profile.jpg" width="150" style="border-radius: 8px;" alt="Sam Emamjinejad"><br>
                <b>Sam Emamjinejad</b>
            </a>
            <p>
                集成传感器设计, 生物医学设备, 数据驱动芯片优化, 低功耗电子器件, 智能硬件
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/puneet-gupta/research-overview.md">
                <img src="research-quickview/ucla/profiles/puneet-gupta-profile.jpg" width="150" style="border-radius: 8px;" alt="Puneet Gupta"><br>
                <b>Puneet Gupta</b>
            </a>
            <p>
                纳米级集成电路设计, 智能制造, 半导体优化, 数据驱动系统开发, 硬件与软件协同优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lei-he/research-overview.md">
                <img src="research-quickview/ucla/profiles/lei-he-profile.jpg" width="150" style="border-radius: 8px;" alt="Lei He"><br>
                <b>Lei He</b>
            </a>
            <p>
                嵌入式系统设计, 集成电路设计与优化, 深度学习硬件设计, 低功耗计算架构, 数据驱动电路设计
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/subramanian-iyer/research-overview.md">
                <img src="research-quickview/ucla/profiles/subramanian-iyer-profile.jpg" width="150" style="border-radius: 8px;" alt="Subramanian Iyer"><br>
                <b>Subramanian Iyer</b>
            </a>
            <p>
                高效半导体设计, 集成系统优化, 纳米技术在IC中的应用, 可持续硬件开发, 功率优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/dejan-markovic/research-overview.md">
                <img src="research-quickview/ucla/profiles/dejan-markovic-profile.jpg" width="150" style="border-radius: 8px;" alt="Dejan Markovic"><br>
                <b>Dejan Markovic</b>
            </a>
            <p>
                神经网络硬件加速, 高性能IC设计, 可编程逻辑设备, 嵌入式系统开发, 低功耗VLSI设计
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/sudhakar-pamarti/research-overview.md">
                <img src="research-quickview/ucla/profiles/sudhakar-pamarti-profile.jpg" width="150" style="border-radius: 8px;" alt="Sudhakar Pamarti"><br>
                <b>Sudhakar Pamarti</b>
            </a>
            <p>
                模拟与数字信号处理, 数据转换器设计, 集成电路优化, 通信系统硬件, 低功耗系统设计
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/behad-razavi/research-overview.md">
                <img src="research-quickview/ucla/profiles/behad-razavi-profile.jpg" width="150" style="border-radius: 8px;" alt="Behad Razavi"><br>
                <b>Behad Razavi</b>
            </a>
            <p>
                高频IC设计, 模拟与混合信号集成电路, 数据转换, 通信与信号处理硬件, 电路优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/john-villasenor/research-overview.md">
                <img src="research-quickview/ucla/profiles/john-villasenor-profile.jpg" width="150" style="border-radius: 8px;" alt="John Villasenor"><br>
                <b>John Villasenor</b>
            </a>
            <p>
                数据科学与硬件, 通信与信号处理, 集成电路优化, 智能设备硬件开发, 网络与信息安全
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/chee-wei-wong/research-overview.md">
                <img src="research-quickview/ucla/profiles/chee-wei-wong-profile.jpg" width="150" style="border-radius: 8px;" alt="Chee Wei Wong"><br>
                <b>Chee Wei Wong</b>
            </a>
            <p>
                光学与光子集成电路, 超高速信号处理, 数据通信硬件, 纳米级IC设计, 量子计算硬件
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/jason-woo/research-overview.md">
                <img src="research-quickview/ucla/profiles/jason-woo-profile.jpg" width="150" style="border-radius: 8px;" alt="Jason Woo"><br>
                <b>Jason Woo</b>
            </a>
            <p>
                半导体技术, 高效IC设计, 智能硬件开发, 数据传输优化, 嵌入式系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/ck-yang/research-overview.md">
                <img src="research-quickview/ucla/profiles/ck-yang-profile.jpg" width="150" style="border-radius: 8px;" alt="C. K. 'Ken' Yang"><br>
                <b>C. K. 'Ken' Yang</b>
            </a>
            <p>
                数字与模拟电路设计, 高速数据转换器, 低功耗IC设计, 数据驱动硬件优化, 集成电路在AI中的应用
            </p>
        </td>
    </tr>
</table>

- **Field**: [Information/Optimization/Computation Theory](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/suhas-diggavi/research-overview.md">
                <img src="research-quickview/ucla/profiles/suhas-diggavi-profile.jpg" width="150" style="border-radius: 8px;" alt="Suhas Diggavi"><br>
                <b>Suhas Diggavi</b>
            </a>
            <p>
                信息理论, 网络优化, 分布式系统, 数据隐私与安全, 机器学习算法, 通信理论
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lara-dolecek/research-overview.md">
                <img src="research-quickview/ucla/profiles/lara-dolecek-profile.jpg" width="150" style="border-radius: 8px;" alt="Lara Dolecek"><br>
                <b>Lara Dolecek</b>
            </a>
            <p>
                编码理论, 信息处理, 数据存储优化, 统计学习, 通信系统优化, 可靠数据传输
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/christina-fragouli/research-overview.md">
                <img src="research-quickview/ucla/profiles/christina-fragouli-profile.jpg" width="150" style="border-radius: 8px;" alt="Christina Fragouli"><br>
                <b>Christina Fragouli</b>
            </a>
            <p>
                网络编码, 数据流优化, 信息理论, 机器学习在网络中的应用, 数据驱动通信
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/achuta-kadambi/research-overview.md">
                <img src="research-quickview/ucla/profiles/achuta-kadambi-profile.jpg" width="150" style="border-radius: 8px;" alt="Achuta Kadambi"><br>
                <b>Achuta Kadambi</b>
            </a>
            <p>
                计算成像, 机器学习优化, 数据处理算法, 人工智能在成像系统中的应用, 图像数据压缩
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/jonathan-kao/research-overview.md">
                <img src="research-quickview/ucla/profiles/jonathan-kao-profile.jpg" width="150" style="border-radius: 8px;" alt="Jonathan Kao"><br>
                <b>Jonathan Kao</b>
            </a>
            <p>
                神经信号处理, 数据驱动优化, 机器学习算法, 信息理论在脑科学中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/di-luo/research-overview.md">
                <img src="research-quickview/ucla/profiles/di-luo-profile.jpg" width="150" style="border-radius: 8px;" alt="Di Luo"><br>
                <b>Di Luo</b>
            </a>
            <p>
                深度学习, 强化学习, 数据分析与优化, 人工智能在计算理论中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/yahya-rahmat-samii/research-overview.md">
                <img src="research-quickview/ucla/profiles/yahya-rahmat-samii-profile.jpg" width="150" style="border-radius: 8px;" alt="Yahya Rahmat-Samii"><br>
                <b>Yahya Rahmat-Samii</b>
            </a>
            <p>
                优化理论, 数据处理, 天线设计中的信息理论, 无线通信优化, 数值计算方法
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/vwani-roychowdhury/research-overview.md">
                <img src="research-quickview/ucla/profiles/vwani-roychowdhury-profile.jpg" width="150" style="border-radius: 8px;" alt="Vwani P. Roychowdhury"><br>
                <b>Vwani P. Roychowdhury</b>
            </a>
            <p>
                数据科学, 量子计算理论, 机器学习优化, 网络科学, 人工智能中的优化算法
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/paulo-tabuada/research-overview.md">
                <img src="research-quickview/ucla/profiles/paulo-tabuada-profile.jpg" width="150" style="border-radius: 8px;" alt="Paulo Tabuada"><br>
                <b>Paulo Tabuada</b>
            </a>
            <p>
                控制与优化, 信息处理算法, 分布式系统理论, 数据驱动自动化, 可靠系统设计
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lieven-vandenberghe/research-overview.md">
                <img src="research-quickview/ucla/profiles/lieven-vandenberghe-profile.jpg" width="150" style="border-radius: 8px;" alt="Lieven Vandenberghe"><br>
                <b>Lieven Vandenberghe</b>
            </a>
            <p>
                优化算法, 数据科学, 机器学习理论, 信息处理优化, 控制系统设计
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/richard-wesel/research-overview.md">
                <img src="research-quickview/ucla/profiles/richard-wesel-profile.jpg" width="150" style="border-radius: 8px;" alt="Richard Wesel"><br>
                <b>Richard Wesel</b>
            </a>
            <p>
                信息理论, 编码优化, 通信系统设计, 数据压缩算法, 可靠传输协议
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lin-yang/research-overview.md">
                <img src="research-quickview/ucla/profiles/lin-yang-profile.jpg" width="150" style="border-radius: 8px;" alt="Lin Yang"><br>
                <b>Lin Yang</b>
            </a>
            <p>
                强化学习, 优化算法, 数据科学, 人工智能中的计算理论, 分布式系统优化
            </p>
        </td>
    </tr>
</table>

- **Field**: [Audio/Image Signal Processing](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/abeer-alwan/research-overview.md">
                <img src="research-quickview/ucla/profiles/abeer-alwan-profile.jpg" width="150" style="border-radius: 8px;" alt="Abeer Alwan"><br>
                <b>Abeer Alwan</b>
            </a>
            <p>
                音频信号处理, 语音识别, 语音合成, 人工智能在语音技术中的应用, 多媒体处理与优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/achuta-kadambi/research-overview.md">
                <img src="research-quickview/ucla/profiles/achuta-kadambi-profile.jpg" width="150" style="border-radius: 8px;" alt="Achuta Kadambi"><br>
                <b>Achuta Kadambi</b>
            </a>
            <p>
                计算成像, 图像处理, 深度学习在视觉系统中的应用, 光学传感, 数据驱动图像优化, 机器学习增强影像分析
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/aydogan-ozcan/research-overview.md">
                <img src="research-quickview/ucla/profiles/aydogan-ozcan-profile.jpg" width="150" style="border-radius: 8px;" alt="Aydogan Ozcan"><br>
                <b>Aydogan Ozcan</b>
            </a>
            <p>
                数字成像, 人工智能驱动图像处理, 光学计算, 纳米技术与影像, 高分辨率图像分析, 生物医学图像优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/vwani-roychowdhury/research-overview.md">
                <img src="research-quickview/ucla/profiles/vwani-roychowdhury-profile.jpg" width="150" style="border-radius: 8px;" alt="Vwani P. Roychowdhury"><br>
                <b>Vwani P. Roychowdhury</b>
            </a>
            <p>
                图像和音频处理, 数据科学与优化, 机器学习算法, 图像压缩与恢复, 多媒体数据处理
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/john-villasenor/research-overview.md">
                <img src="research-quickview/ucla/profiles/john-villasenor-profile.jpg" width="150" style="border-radius: 8px;" alt="John Villasenor"><br>
                <b>John Villasenor</b>
            </a>
            <p>
                多媒体信号处理, 数据安全与音视频应用, 图像与视频优化, 人工智能在图像处理中的应用, 网络化媒体分析
            </p>
        </td>
    </tr>
</table>

- **Field**: [Cyberphysical Systems, Control, and Robotics](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/robert-candler/research-overview.md">
                <img src="research-quickview/ucla/profiles/robert-candler-profile.jpg" width="150" style="border-radius: 8px;" alt="Robert Candler"><br>
                <b>Robert Candler</b>
            </a>
            <p>
                微机电系统 (MEMS), 传感与执行器技术, 智能控制系统, 低功耗嵌入式系统, 物联网硬件设计
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/xiaofan-cui/research-overview.md">
                <img src="research-quickview/ucla/profiles/xiaofan-cui-profile.jpg" width="150" style="border-radius: 8px;" alt="Xiaofan Cui"><br>
                <b>Xiaofan Cui</b>
            </a>
            <p>
                网络化控制系统, 分布式机器人系统, 数据驱动控制, 智能电网与能源系统, 传感器网络优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/suhas-diggavi/research-overview.md">
                <img src="research-quickview/ucla/profiles/suhas-diggavi-profile.jpg" width="150" style="border-radius: 8px;" alt="Suhas Diggavi"><br>
                <b>Suhas Diggavi</b>
            </a>
            <p>
                分布式系统控制, 网络安全, 数据隐私, 信息理论, 机器学习在控制系统中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/sam-emamjinejad/research-overview.md">
                <img src="research-quickview/ucla/profiles/sam-emamjinejad-profile.jpg" width="150" style="border-radius: 8px;" alt="Sam Emamjinejad"><br>
                <b>Sam Emamjinejad</b>
            </a>
            <p>
                生物医学设备, 传感与控制, 数据驱动健康监测, 智能硬件开发, 多功能集成系统
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/christina-fragouli/research-overview.md">
                <img src="research-quickview/ucla/profiles/christina-fragouli-profile.jpg" width="150" style="border-radius: 8px;" alt="Christina Fragouli"><br>
                <b>Christina Fragouli</b>
            </a>
            <p>
                网络编码, 分布式系统优化, 数据流控制, 信息理论在网络化系统中的应用, 智能网络设计
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/achuta-kadambi/research-overview.md">
                <img src="research-quickview/ucla/profiles/achuta-kadambi-profile.jpg" width="150" style="border-radius: 8px;" alt="Achuta Kadambi"><br>
                <b>Achuta Kadambi</b>
            </a>
            <p>
                机器人视觉系统, 计算成像, 智能传感器设计, 数据驱动控制优化, 机器学习在机器人系统中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/jonathan-kao/research-overview.md">
                <img src="research-quickview/ucla/profiles/jonathan-kao-profile.jpg" width="150" style="border-radius: 8px;" alt="Jonathan Kao"><br>
                <b>Jonathan Kao</b>
            </a>
            <p>
                神经科学中的控制理论, 数据优化, 智能传感系统, 人工智能在生物控制系统中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/ankur-mehta/research-overview.md">
                <img src="research-quickview/ucla/profiles/ankur-mehta-profile.jpg" width="150" style="border-radius: 8px;" alt="Ankur Mehta"><br>
                <b>Ankur Mehta</b>
            </a>
            <p>
                自主机器人, 网络化嵌入式系统, 分布式控制, 数据驱动优化, 低成本机器人设计
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/vwani-roychowdhury/research-overview.md">
                <img src="research-quickview/ucla/profiles/vwani-roychowdhury-profile.jpg" width="150" style="border-radius: 8px;" alt="Vwani P. Roychowdhury"><br>
                <b>Vwani P. Roychowdhury</b>
            </a>
            <p>
                数据科学, 网络优化, 机器学习与控制, 智能系统中的分布式计算, 信息流管理
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/nader-sehatbakhsh/research-overview.md">
                <img src="research-quickview/ucla/profiles/nader-sehatbakhsh-profile.jpg" width="150" style="border-radius: 8px;" alt="Nader Sehatbakhsh"><br>
                <b>Nader Sehatbakhsh</b>
            </a>
            <p>
                嵌入式系统安全, 分布式控制, 硬件优化, 数据隐私与保护, 智能硬件系统设计
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/mani-srivastava/research-overview.md">
                <img src="research-quickview/ucla/profiles/mani-srivastava-profile.jpg" width="150" style="border-radius: 8px;" alt="Mani Srivastava"><br>
                <b>Mani Srivastava</b>
            </a>
            <p>
                物联网系统, 嵌入式控制, 数据隐私与优化, 网络化传感器系统, 分布式系统安全
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/paulo-tabuada/research-overview.md">
                <img src="research-quickview/ucla/profiles/paulo-tabuada-profile.jpg" width="150" style="border-radius: 8px;" alt="Paulo Tabuada"><br>
                <b>Paulo Tabuada</b>
            </a>
            <p>
                控制系统理论, 智能自动化, 数据驱动优化, 网络化控制系统, 分布式机器人设计
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/yuan-tian/research-overview.md">
                <img src="research-quickview/ucla/profiles/yuan-tian-profile.jpg" width="150" style="border-radius: 8px;" alt="Yuan Tian"><br>
                <b>Yuan Tian</b>
            </a>
            <p>
                数据隐私与控制, 网络化系统优化, 智能设备设计, 控制系统中的人工智能, 系统安全与优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lieven-vandenberghe/research-overview.md">
                <img src="research-quickview/ucla/profiles/lieven-vandenberghe-profile.jpg" width="150" style="border-radius: 8px;" alt="Lieven Vandenberghe"><br>
                <b>Lieven Vandenberghe</b>
            </a>
            <p>
                优化算法, 控制理论, 数据分析与系统优化, 分布式控制, 网络化系统设计
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/john-villasenor/research-overview.md">
                <img src="research-quickview/ucla/profiles/john-villasenor-profile.jpg" width="150" style="border-radius: 8px;" alt="John Villasenor"><br>
                <b>John Villasenor</b>
            </a>
            <p>
                网络化控制, 数据优化, 智能设备系统, 嵌入式硬件设计, 系统安全与人工智能
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/ck-yang/research-overview.md">
                <img src="research-quickview/ucla/profiles/ck-yang-profile.jpg" width="150" style="border-radius: 8px;" alt="C. K. 'Ken' Yang"><br>
                <b>C. K. 'Ken' Yang</b>
            </a>
            <p>
                数字与模拟控制, 嵌入式系统设计, 高速传感器接口, 智能硬件优化, 控制系统中的集成电路
            </p>
        </td>
    </tr>
</table>

- **Field**: [Data Communication and Networking](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/danijela-cabric/research-overview.md">
                <img src="research-quickview/ucla/profiles/danijela-cabric-profile.jpg" width="150" style="border-radius: 8px;" alt="Danijela Cabric"><br>
                <b>Danijela Cabric</b>
            </a>
            <p>
                无线通信, 频谱共享, 智能无线网络, 分布式通信系统, 数据优化与网络化应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/suhas-diggavi/research-overview.md">
                <img src="research-quickview/ucla/profiles/suhas-diggavi-profile.jpg" width="150" style="border-radius: 8px;" alt="Suhas Diggavi"><br>
                <b>Suhas Diggavi</b>
            </a>
            <p>
                信息理论, 分布式网络优化, 数据隐私与安全, 通信协议优化, 数据驱动通信系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lara-dolecek/research-overview.md">
                <img src="research-quickview/ucla/profiles/lara-dolecek-profile.jpg" width="150" style="border-radius: 8px;" alt="Lara Dolecek"><br>
                <b>Lara Dolecek</b>
            </a>
            <p>
                编码理论, 数据存储优化, 通信系统设计, 高效网络优化, 统计学习在通信中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/christina-fragouli/research-overview.md">
                <img src="research-quickview/ucla/profiles/christina-fragouli-profile.jpg" width="150" style="border-radius: 8px;" alt="Christina Fragouli"><br>
                <b>Christina Fragouli</b>
            </a>
            <p>
                网络编码, 数据流优化, 信息理论, 机器学习在通信中的应用, 智能网络设计
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/liz-izhikevich/research-overview.md">
                <img src="research-quickview/ucla/profiles/liz-izhikevich-profile.jpg" width="150" style="border-radius: 8px;" alt="Liz Izhikevich"><br>
                <b>Liz Izhikevich</b>
            </a>
            <p>
                通信系统安全, 数据隐私, 网络优化, 智能通信协议设计, 数据共享中的隐私保护
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/sudhakar-pamarti/research-overview.md">
                <img src="research-quickview/ucla/profiles/sudhakar-pamarti-profile.jpg" width="150" style="border-radius: 8px;" alt="Sudhakar Pamarti"><br>
                <b>Sudhakar Pamarti</b>
            </a>
            <p>
                模拟与数字信号处理, 通信电路设计, 数据转换器优化, 高效通信系统硬件, 低功耗网络硬件
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/ian-roberts/research-overview.md">
                <img src="research-quickview/ucla/profiles/ian-roberts-profile.jpg" width="150" style="border-radius: 8px;" alt="Ian Roberts"><br>
                <b>Ian Roberts</b>
            </a>
            <p>
                无线通信, 网络协议优化, 数据流控制, 智能网络设计, 高效数据通信硬件
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/mani-srivastava/research-overview.md">
                <img src="research-quickview/ucla/profiles/mani-srivastava-profile.jpg" width="150" style="border-radius: 8px;" alt="Mani Srivastava"><br>
                <b>Mani Srivastava</b>
            </a>
            <p>
                数据通信与网络安全, 分布式网络优化, 物联网通信, 数据隐私与优化, 网络传感器系统
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/john-villasenor/research-overview.md">
                <img src="research-quickview/ucla/profiles/john-villasenor-profile.jpg" width="150" style="border-radius: 8px;" alt="John Villasenor"><br>
                <b>John Villasenor</b>
            </a>
            <p>
                通信系统政策, 网络安全, 数据优化, 多媒体通信, 智能网络与数据管理
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/richard-wesel/research-overview.md">
                <img src="research-quickview/ucla/profiles/richard-wesel-profile.jpg" width="150" style="border-radius: 8px;" alt="Richard Wesel"><br>
                <b>Richard Wesel</b>
            </a>
            <p>
                信息理论, 通信协议设计, 数据压缩与传输, 高效网络优化, 可靠通信系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/chee-wei-wong/research-overview.md">
                <img src="research-quickview/ucla/profiles/chee-wei-wong-profile.jpg" width="150" style="border-radius: 8px;" alt="Chee Wei Wong"><br>
                <b>Chee Wei Wong</b>
            </a>
            <p>
                光通信, 数据传输优化, 高速网络设计, 量子通信, 智能通信系统开发
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/ck-yang/research-overview.md">
                <img src="research-quickview/ucla/profiles/ck-yang-profile.jpg" width="150" style="border-radius: 8px;" alt="C. K. 'Ken' Yang"><br>
                <b>C. K. 'Ken' Yang</b>
            </a>
            <p>
                通信电路设计, 高速数据传输, 网络硬件优化, 数据驱动通信系统, 嵌入式通信硬件
            </p>
        </td>
    </tr>
</table>

- **Field**: [Photonics, Optics, and Plasma Electronics](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/robert-candler/research-overview.md">
                <img src="research-quickview/ucla/profiles/robert-candler-profile.jpg" width="150" style="border-radius: 8px;" alt="Robert Candler"><br>
                <b>Robert Candler</b>
            </a>
            <p>
                微机电系统 (MEMS), 光学传感器, 激光干涉仪设计, 高精度光子系统, 纳米级光学设备
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/sergio-carbajo/research-overview.md">
                <img src="research-quickview/ucla/profiles/sergio-carbajo-profile.jpg" width="150" style="border-radius: 8px;" alt="Sergio Carbajo"><br>
                <b>Sergio Carbajo</b>
            </a>
            <p>
                激光物理, 超快光子学, 等离子体光学, 高功率激光系统, 数据驱动光学优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/frank-chang/research-overview.md">
                <img src="research-quickview/ucla/profiles/frank-chang-profile.jpg" width="150" style="border-radius: 8px;" alt="Frank Chang"><br>
                <b>Frank Chang</b>
            </a>
            <p>
                光通信, 微波与毫米波系统, 光子集成电路设计, 高速光网络, 数据驱动光学系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/mona-jarrahi/research-overview.md">
                <img src="research-quickview/ucla/profiles/mona-jarrahi-profile.jpg" width="150" style="border-radius: 8px;" alt="Mona Jarrahi"><br>
                <b>Mona Jarrahi</b>
            </a>
            <p>
                太赫兹技术, 光子学, 光电探测器, 激光与光学成像, 数据驱动太赫兹成像系统
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/chandrashekhar-joshi/research-overview.md">
                <img src="research-quickview/ucla/profiles/chandrashekhar-joshi-profile.jpg" width="150" style="border-radius: 8px;" alt="Chandrashekhar Joshi"><br>
                <b>Chandrashekhar Joshi</b>
            </a>
            <p>
                等离子体物理, 激光等离子体相互作用, 高功率激光驱动, 光电系统开发, 高能量密度等离子体技术
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/jia-ming-liu/research-overview.md">
                <img src="research-quickview/ucla/profiles/jia-ming-liu-profile.jpg" width="150" style="border-radius: 8px;" alt="Jia-Ming Liu"><br>
                <b>Jia-Ming Liu</b>
            </a>
            <p>
                激光物理, 光子集成电路, 高速光信号处理, 纳米光学设备, 光通信系统优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/di-luo/research-overview.md">
                <img src="research-quickview/ucla/profiles/di-luo-profile.jpg" width="150" style="border-radius: 8px;" alt="Di Luo"><br>
                <b>Di Luo</b>
            </a>
            <p>
                超快光学, 光子学优化, 数据驱动光子系统, 纳米级光学设计, 光电系统自动化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/aydogan-ozcan/research-overview.md">
                <img src="research-quickview/ucla/profiles/aydogan-ozcan-profile.jpg" width="150" style="border-radius: 8px;" alt="Aydogan Ozcan"><br>
                <b>Aydogan Ozcan</b>
            </a>
            <p>
                数字光学, 光子传感器, 高分辨率成像, 纳米技术与光学, 人工智能驱动光学设计
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/kang-wang/research-overview.md">
                <img src="research-quickview/ucla/profiles/kang-wang-profile.jpg" width="150" style="border-radius: 8px;" alt="Kang L. Wang"><br>
                <b>Kang L. Wang</b>
            </a>
            <p>
                光子材料, 光子学与电子学集成, 高速光电转换, 纳米光学技术, 数据驱动光电优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/benjamin-williams/research-overview.md">
                <img src="research-quickview/ucla/profiles/benjamin-williams-profile.jpg" width="150" style="border-radius: 8px;" alt="Benjamin Williams"><br>
                <b>Benjamin Williams</b>
            </a>
            <p>
                太赫兹物理, 光子系统设计, 高效光电设备, 激光与光学集成, 太赫兹成像技术
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/chee-wei-wong/research-overview.md">
                <img src="research-quickview/ucla/profiles/chee-wei-wong-profile.jpg" width="150" style="border-radius: 8px;" alt="Chee Wei Wong"><br>
                <b>Chee Wei Wong</b>
            </a>
            <p>
                光子学与光学集成, 纳米光学设备, 高速光信号处理, 数据通信光学优化, 量子光学技术
            </p>
        </td>
    </tr>
</table>

- **Field**: [Quantum Science](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/elaheh-ahmadi/research-overview.md">
                <img src="research-quickview/ucla/profiles/elaheh-ahmadi-profile.jpg" width="150" style="border-radius: 8px;" alt="Elaheh Ahmadi"><br>
                <b>Elaheh Ahmadi</b>
            </a>
            <p>
                量子材料, 半导体设备中的量子现象, 纳米材料, 高性能量子器件设计, 数据驱动量子优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/robert-candler/research-overview.md">
                <img src="research-quickview/ucla/profiles/robert-candler-profile.jpg" width="150" style="border-radius: 8px;" alt="Robert Candler"><br>
                <b>Robert Candler</b>
            </a>
            <p>
                量子设备制造, 纳米级量子传感器, MEMS在量子系统中的应用, 高精度量子测量
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/sergio-carbajo/research-overview.md">
                <img src="research-quickview/ucla/profiles/sergio-carbajo-profile.jpg" width="150" style="border-radius: 8px;" alt="Sergio Carbajo"><br>
                <b>Sergio Carbajo</b>
            </a>
            <p>
                量子光学, 超快激光在量子系统中的应用, 高能量量子光子学, 量子态控制与优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/lara-dolecek/research-overview.md">
                <img src="research-quickview/ucla/profiles/lara-dolecek-profile.jpg" width="150" style="border-radius: 8px;" alt="Lara Dolecek"><br>
                <b>Lara Dolecek</b>
            </a>
            <p>
                量子信息理论, 编码优化, 量子通信, 数据存储中的量子现象, 量子纠错
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/chandrashekhar-joshi/research-overview.md">
                <img src="research-quickview/ucla/profiles/chandrashekhar-joshi-profile.jpg" width="150" style="border-radius: 8px;" alt="Chandrashekhar Joshi"><br>
                <b>Chandrashekhar Joshi</b>
            </a>
            <p>
                等离子体量子现象, 高能量量子系统, 激光等离子体交互, 高功率量子设备
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/di-luo/research-overview.md">
                <img src="research-quickview/ucla/profiles/di-luo-profile.jpg" width="150" style="border-radius: 8px;" alt="Di Luo"><br>
                <b>Di Luo</b>
            </a>
            <p>
                量子计算, 数据优化中的量子算法, 量子态模拟, 量子硬件设计, 量子增强人工智能
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/vwani-roychowdhury/research-overview.md">
                <img src="research-quickview/ucla/profiles/vwani-roychowdhury-profile.jpg" width="150" style="border-radius: 8px;" alt="Vwani P. Roychowdhury"><br>
                <b>Vwani P. Roychowdhury</b>
            </a>
            <p>
                量子信息科学, 网络优化中的量子算法, 数据科学与量子计算, 量子纠错, 量子机器学习
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/kang-wang/research-overview.md">
                <img src="research-quickview/ucla/profiles/kang-wang-profile.jpg" width="150" style="border-radius: 8px;" alt="Kang L. Wang"><br>
                <b>Kang L. Wang</b>
            </a>
            <p>
                量子材料, 量子电子器件, 自旋量子态, 量子信息处理硬件, 量子存储系统
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/benjamin-williams/research-overview.md">
                <img src="research-quickview/ucla/profiles/benjamin-williams-profile.jpg" width="150" style="border-radius: 8px;" alt="Benjamin Williams"><br>
                <b>Benjamin Williams</b>
            </a>
            <p>
                太赫兹量子设备, 光子与量子集成, 高速量子信号处理, 量子材料设计, 太赫兹量子传感器
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/chee-wei-wong/research-overview.md">
                <img src="research-quickview/ucla/profiles/chee-wei-wong-profile.jpg" width="150" style="border-radius: 8px;" alt="Chee Wei Wong"><br>
                <b>Chee Wei Wong</b>
            </a>
            <p>
                量子光子学, 高速量子通信, 纳米光子设备, 量子态优化, 量子光学集成
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/leo-zhou/research-overview.md">
                <img src="research-quickview/ucla/profiles/leo-zhou-profile.jpg" width="150" style="border-radius: 8px;" alt="Leo Zhou"><br>
                <b>Leo Zhou</b>
            </a>
            <p>
                量子算法, 量子信息科学, 量子硬件设计, 高效量子态模拟, 数据科学中的量子应用
            </p>
        </td>
    </tr>
</table>

- **Field**: [Semiconductors and Packaging Technology](https://www.ee.ucla.edu/ucla-ee-research/)
  <table>
    <tr>
      <td align="center">
        <a href="research-quickview/ucla/ece/elaheh-ahmadi/research-overview.md">
          <img src="research-quickview/ucla/profiles/elaheh-ahmadi-profile.jpg" width="150" style="border-radius: 8px;" alt="Elaheh Ahmadi"><br>
          <b>Elaheh Ahmadi</b>
        </a>
        <p>芯片掺杂，分子束外延，半导体材料, 功率电子, 纳米器件设计, 高性能晶体管, 数据驱动半导体优化</p>
      </td>
      <td align="center">
        <a href="research-quickview/ucla/ece/robert-candler/research-overview.md">
          <img src="research-quickview/ucla/profiles/robert-candler-profile.jpg" width="150" style="border-radius: 8px;" alt="Robert Candler"><br>
          <b>Robert Candler</b>
        </a>
        <p>纳米磁铁材料设计制造，生物医学应用</p>
      </td>
      <td align="center">
        <a href="research-quickview/ucla/ece/frank-chang/research-overview.md">
          <img src="research-quickview/ucla/profiles/frank-chang-profile.jpg" width="150" style="border-radius: 8px;" alt="Frank Chang"><br>
          <b>Frank Chang</b>
        </a>
        <p>射频芯片设计与集成，高速无线通信, 微波与毫米波系统</p>
      </td>
      <td align="center">
        <a href="research-quickview/ucla/ece/sam-emamjinejad/research-overview.md">
          <img src="research-quickview/ucla/profiles/sam-emamjinejad-profile.jpg" width="150" style="border-radius: 8px;" alt="Sam Emamjinejad"><br>
          <b>Sam Emamjinejad</b>
        </a>
        <p>可穿戴生物电子传感器、健康监测</p>
      </td>
    </tr>
    <tr>
      <td align="center">
        <a href="research-quickview/ucla/ece/puneet-gupta/research-overview.md">
          <img src="research-quickview/ucla/profiles/puneet-gupta-profile.jpg" width="150" style="border-radius: 8px;" alt="Puneet Gupta"><br>
          <b>Puneet Gupta</b>
        </a>
        <p>芯片设计、光子计算和存储</p>
      </td>
      <td align="center">
        <a href="research-quickview/ucla/ece/subramanian-iyer/research-overview.md">
          <img src="research-quickview/ucla/profiles/subramanian-iyer-profile.jpg" width="150" style="border-radius: 8px;" alt="Subramanian Iyer"><br>
          <b>Subramanian Iyer</b>
        </a>
        <p>芯片封装和光刻</p>
      </td>
      <td align="center">
        <a href="research-quickview/ucla/ece/sudhakar-pamarti/research-overview.md">
          <img src="research-quickview/ucla/profiles/sudhakar-pamarti-profile.jpg" width="150" style="border-radius: 8px;" alt="Sudhakar Pamarti"><br>
          <b>Sudhakar Pamarti</b>
        </a>
        <p>低功耗芯片设计，半导体器件开发</p>
      </td>
      <td align="center">
        <a href="research-quickview/ucla/ece/kang-wang/research-overview.md">
          <img src="research-quickview/ucla/profiles/kang-wang-profile.jpg" width="150" style="border-radius: 8px;" alt="Kang L. Wang"><br>
          <b>Kang L. Wang</b>
        </a>
        <p>量子材料，自旋电子器，磁性半导体</p>
      </td>
    </tr>
    <tr>
      <td align="center">
        <a href="research-quickview/ucla/ece/benjamin-williams/research-overview.md">
          <img src="research-quickview/ucla/profiles/benjamin-williams-profile.jpg" width="150" style="border-radius: 8px;" alt="Benjamin Williams"><br>
          <b>Benjamin Williams</b>
        </a>
        <p>太赫兹半导体技术，量子级联激光器，超表面激光器</p>
      </td>
      <td align="center">
        <a href="research-quickview/ucla/ece/chee-wei-wong/research-overview.md">
          <img src="research-quickview/ucla/profiles/chee-wei-wong-profile.jpg" width="150" style="border-radius: 8px;" alt="Chee Wei Wong"><br>
          <b>Chee Wei Wong</b>
        </a>
        <p>频率梳，微腔光子器件，量子通信，光子与半导体集成, 纳米级光电器件</p>
      </td>
      <td align="center">
        <a href="research-quickview/ucla/ece/jason-woo/research-overview.md">
          <img src="research-quickview/ucla/profiles/jason-woo-profile.jpg" width="150" style="border-radius: 8px;" alt="Jason Woo"><br>
          <b>Jason Woo</b>
        </a>
        <p>疑似退休，半导体制造技术, 集成电路设计, 封装与测试优化, 高效器件开发, 智能封装解决方案</p>
      </td>
      <td align="center">
        <a href="research-quickview/ucla/ece/ken-yang/research-overview.md">
          <img src="research-quickview/ucla/profiles/ck-yang-profile.jpg" width="150" style="border-radius: 8px;" alt="C. K. 'Ken' Yang"><br>
          <b>C. K. "Ken" Yang</b>
        </a>
        <p>无课题组网站，集成电路设计, 数据驱动封装优化, 高频电子器件, 半导体设备开发, 高效硬件设计</p>
      </td>
    </tr>
    <tr>
      <td align="center">
        <a href="research-quickview/ucla/ece/mona-jarrahi/research-overview.md">
          <img src="research-quickview/ucla/profiles/mona-jarrahi-profile.jpg" width="150" style="border-radius: 8px;" alt="Mona Jarrahi"><br>
          <b>Mona Jarrahi</b>
        </a>
        <p>太赫兹成像，光学处理，光子计算，太赫兹半导体技术, 纳米级光子封装</p>
      </td>
    </tr>
  </table>
  
- **Field**: [Electromagnetics/Antennas – Microwave and mmWave Systems](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/elaheh-ahmadi/research-overview.md">
                <img src="research-quickview/ucla/profiles/elaheh-ahmadi-profile.jpg" width="150" style="border-radius: 8px;" alt="Elaheh Ahmadi"><br>
                <b>Elaheh Ahmadi</b>
            </a>
            <p>
                电磁材料, 微波与毫米波器件, 天线设计, 高性能材料优化, 频率选择表面, 纳米技术在电磁系统中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/aydin-babakhani/research-overview.md">
                <img src="research-quickview/ucla/profiles/aydin-babakhani-profile.jpg" width="150" style="border-radius: 8px;" alt="Aydin Babakhani"><br>
                <b>Aydin Babakhani</b>
            </a>
            <p>
                毫米波与太赫兹系统, 天线阵列设计, 高速无线通信, 低功耗射频传感器, 射频集成电路
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/robert-candler/research-overview.md">
                <img src="research-quickview/ucla/profiles/robert-candler-profile.jpg" width="150" style="border-radius: 8px;" alt="Robert Candler"><br>
                <b>Robert Candler</b>
            </a>
            <p>
                微机电系统 (MEMS), 微波设备, 纳米级天线设计, 高功率射频系统, 微波传感器优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/frank-chang/research-overview.md">
                <img src="research-quickview/ucla/profiles/frank-chang-profile.jpg" width="150" style="border-radius: 8px;" alt="Frank Chang"><br>
                <b>Frank Chang</b>
            </a>
            <p>
                微波与毫米波系统, 高速通信, 集成电路设计, 射频系统优化, 天线设计与实现
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/sam-emamjinejad/research-overview.md">
                <img src="research-quickview/ucla/profiles/sam-emamjinejad-profile.jpg" width="150" style="border-radius: 8px;" alt="Sam Emamjinejad"><br>
                <b>Sam Emamjinejad</b>
            </a>
            <p>
                微波传感器设计, 射频系统优化, 数据驱动天线系统, 医疗射频设备, 微波与毫米波在生物医学中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/subramanian-iyer/research-overview.md">
                <img src="research-quickview/ucla/profiles/subramanian-iyer-profile.jpg" width="150" style="border-radius: 8px;" alt="Subramanian Iyer"><br>
                <b>Subramanian Iyer</b>
            </a>
            <p>
                射频系统集成, 高效天线设计, 微波设备优化, 纳米技术在天线中的应用, 数据驱动微波优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/mona-jarrahi/research-overview.md">
                <img src="research-quickview/ucla/profiles/mona-jarrahi-profile.jpg" width="150" style="border-radius: 8px;" alt="Mona Jarrahi"><br>
                <b>Mona Jarrahi</b>
            </a>
            <p>
                太赫兹系统设计, 电磁与光子技术, 微波与毫米波天线, 高频电子设备, 数据驱动太赫兹优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/aydogan-ozcan/research-overview.md">
                <img src="research-quickview/ucla/profiles/aydogan-ozcan-profile.jpg" width="150" style="border-radius: 8px;" alt="Aydogan Ozcan"><br>
                <b>Aydogan Ozcan</b>
            </a>
            <p>
                纳米光子学, 微波与毫米波应用, 高分辨率天线成像, 数据驱动光电优化, 医疗成像技术
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/yahya-rahmat-samii/research-overview.md">
                <img src="research-quickview/ucla/profiles/yahya-rahmat-samii-profile.jpg" width="150" style="border-radius: 8px;" alt="Yahya Rahmat-Samii"><br>
                <b>Yahya Rahmat-Samii</b>
            </a>
            <p>
                天线设计, 电磁理论, 微波与毫米波系统, 太空通信天线, 天线优化与分析
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/kang-wang/research-overview.md">
                <img src="research-quickview/ucla/profiles/kang-wang-profile.jpg" width="150" style="border-radius: 8px;" alt="Kang L. Wang"><br>
                <b>Kang L. Wang</b>
            </a>
            <p>
                微波材料, 磁性天线, 高速射频设备, 自旋电子学在射频中的应用, 高效天线优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/yuanxun-wang/research-overview.md">
                <img src="research-quickview/ucla/profiles/yuanxun-wang-profile.jpg" width="150" style="border-radius: 8px;" alt="Yuanxun Ethan Wang"><br>
                <b>Yuanxun Ethan Wang</b>
            </a>
            <p>
                天线优化, 微波系统设计, 射频传感器, 数据驱动射频优化, 高速无线通信系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/benjamin-williams/research-overview.md">
                <img src="research-quickview/ucla/profiles/benjamin-williams-profile.jpg" width="150" style="border-radius: 8px;" alt="Benjamin Williams"><br>
                <b>Benjamin Williams</b>
            </a>
            <p>
                太赫兹物理, 高频天线设计, 微波与毫米波系统, 光子与电磁集成, 太赫兹传感器优化
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/chee-wei-wong/research-overview.md">
                <img src="research-quickview/ucla/profiles/chee-wei-wong-profile.jpg" width="150" style="border-radius: 8px;" alt="Chee Wei Wong"><br>
                <b>Chee Wei Wong</b>
            </a>
            <p>
                光子学与天线集成, 高频电磁系统, 微波与光通信, 数据优化天线系统, 光电系统设计
            </p>
        </td>
    </tr>
</table>

- **Field**: [Biomedical Systems and Applications](https://www.ee.ucla.edu/ucla-ee-research/)
<table>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/aydin-babakhani/research-overview.md">
                <img src="research-quickview/ucla/profiles/aydin-babakhani-profile.jpg" width="150" style="border-radius: 8px;" alt="Aydin Babakhani"><br>
                <b>Aydin Babakhani</b>
            </a>
            <p>
                生物医学传感器, 高频无线医疗设备, 微波与毫米波在生物医学中的应用, 数据驱动医疗优化, 智能健康监测系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/robert-candler/research-overview.md">
                <img src="research-quickview/ucla/profiles/robert-candler-profile.jpg" width="150" style="border-radius: 8px;" alt="Robert Candler"><br>
                <b>Robert Candler</b>
            </a>
            <p>
                生物医学设备, 微机电系统 (MEMS), 纳米级医疗传感器, 高精度成像技术, 智能健康硬件
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/suhas-diggavi/research-overview.md">
                <img src="research-quickview/ucla/profiles/suhas-diggavi-profile.jpg" width="150" style="border-radius: 8px;" alt="Suhas Diggavi"><br>
                <b>Suhas Diggavi</b>
            </a>
            <p>
                数据隐私与健康, 分布式医疗优化, 生物医学信号处理, 智能健康数据分析, 通信系统在医疗中的应用
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/sam-emamjinejad/research-overview.md">
                <img src="research-quickview/ucla/profiles/sam-emamjinejad-profile.jpg" width="150" style="border-radius: 8px;" alt="Sam Emamjinejad"><br>
                <b>Sam Emamjinejad</b>
            </a>
            <p>
                智能医疗设备, 健康监测传感器, 数据驱动生物医学系统, 个性化医疗优化, 生物医学信号处理
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/subramanian-iyer/research-overview.md">
                <img src="research-quickview/ucla/profiles/subramanian-iyer-profile.jpg" width="150" style="border-radius: 8px;" alt="Subramanian Iyer"><br>
                <b>Subramanian Iyer</b>
            </a>
            <p>
                生物集成设备, 半导体在医疗中的应用, 纳米技术优化, 生物医疗硬件开发, 微尺度医疗系统
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/achuta-kadambi/research-overview.md">
                <img src="research-quickview/ucla/profiles/achuta-kadambi-profile.jpg" width="150" style="border-radius: 8px;" alt="Achuta Kadambi"><br>
                <b>Achuta Kadambi</b>
            </a>
            <p>
                医疗成像系统, 光学传感器, 数据驱动医疗优化, 人工智能在生物医学中的应用, 生物成像技术
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/jonathan-kao/research-overview.md">
                <img src="research-quickview/ucla/profiles/jonathan-kao-profile.jpg" width="150" style="border-radius: 8px;" alt="Jonathan Kao"><br>
                <b>Jonathan Kao</b>
            </a>
            <p>
                神经信号处理, 脑机接口, 智能医疗数据分析, 生物医学算法优化, 精准医疗技术
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/dejan-markovic/research-overview.md">
                <img src="research-quickview/ucla/profiles/dejan-markovic-profile.jpg" width="150" style="border-radius: 8px;" alt="Dejan Markovic"><br>
                <b>Dejan Markovic</b>
            </a>
            <p>
                高效医疗硬件设计, 生物医学信号处理, 嵌入式医疗系统, 数据驱动健康设备, 智能医疗传感器
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/aydogan-ozcan/research-overview.md">
                <img src="research-quickview/ucla/profiles/aydogan-ozcan-profile.jpg" width="150" style="border-radius: 8px;" alt="Aydogan Ozcan"><br>
                <b>Aydogan Ozcan</b>
            </a>
            <p>
                数字医疗成像, 光学传感器, 生物医学成像技术, 人工智能驱动医疗设备, 高分辨率成像优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/yahya-rahmat-samii/research-overview.md">
                <img src="research-quickview/ucla/profiles/yahya-rahmat-samii-profile.jpg" width="150" style="border-radius: 8px;" alt="Yahya Rahmat-Samii"><br>
                <b>Yahya Rahmat-Samii</b>
            </a>
            <p>
                天线与医疗设备, 电磁学在生物医学中的应用, 数据驱动医疗优化, 高效医疗设备开发, 医疗设备的无线通信
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/vwani-roychowdhury/research-overview.md">
                <img src="research-quickview/ucla/profiles/vwani-roychowdhury-profile.jpg" width="150" style="border-radius: 8px;" alt="Vwani P. Roychowdhury"><br>
                <b>Vwani P. Roychowdhury</b>
            </a>
            <p>
                医疗数据科学, 机器学习在医疗中的应用, 网络化医疗系统, 数据驱动生物医学分析, 智能医疗优化
            </p>
        </td>
        <td align="center">
            <a href="research-quickview/ucla/ece/mani-srivastava/research-overview.md">
                <img src="research-quickview/ucla/profiles/mani-srivastava-profile.jpg" width="150" style="border-radius: 8px;" alt="Mani Srivastava"><br>
                <b>Mani Srivastava</b>
            </a>
            <p>
                医疗物联网, 数据隐私与安全, 分布式医疗系统, 嵌入式医疗设备, 智能健康监控
            </p>
        </td>
    </tr>
    <tr>
        <td align="center">
            <a href="research-quickview/ucla/ece/yuan-tian/research-overview.md">
                <img src="research-quickview/ucla/profiles/yuan-tian-profile.jpg" width="150" style="border-radius: 8px;" alt="Yuan Tian"><br>
                <b>Yuan Tian</b>
            </a>
            <p>
                医疗数据隐私, 网络化医疗设备, 智能医疗设备优化, 人工智能在健康监测中的应用, 数据驱动医疗安全
            </p>
        </td>
    </tr>
</table>
